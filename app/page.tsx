...
import {
  type PodcastEpisode as PodcastEpisode,
  podcastCategories,
} from "@/types/podcast";
import {
...
export default function Home() {
  ...
  const setRandomEpisode = async () => {
    const eligibleEpisodes = episodes.filter((episode) =>
      enabledCategories.includes(episode.category.id),
    );
        ...
        <div className="grow flex items-center justify-center min-h-48">
          {isEpisodeListLoading ? (
            ...
            <div className="text-center p-6 max-w-md">
              ...
              <Button
                variant="outline"
                onClick={() => loadPodcastEpisodes(true)}
                className="text-sm"
              >
                Try Again
              </Button>
